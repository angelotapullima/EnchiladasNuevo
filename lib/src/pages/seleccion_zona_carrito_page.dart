...
class SeleccionZona extends StatelessWidget {
      ...
      body: StreamBuilder(
        ...
        builder: (BuildContext context, AsyncSnapshot<List<Zona>> snapshot) {
          if (snapshot.hasData) {
            return Stack(
              children: <Widget>[
                _stackTarjetas(context, snapshot.data, responsive,argumento),
                Container(
  ...
  Widget _stackTarjetas(
      BuildContext context, List<Zona> zona, Responsive responsive,String argumento) {
    return Stack(
      children: <Widget>[
        _backgroundImage(zona, responsive),
        _carousel(context, zona, responsive,argumento),
        _button(context, zona, responsive,argumento),
      ],
    );
  }

  Widget _button(BuildContext context, List<Zona> zona, Responsive responsive,String argumento) {
      ...
      child: ButtonTheme(
        ...
        child: RaisedButton(
          ...
          onPressed: () {
            Zona zonasss = Zona();
            zonasss.idZona =
                zona[(_scrollController.offset ~/ size.width)].idZona;
            zonasss.idProducto =
                zona[(_scrollController.offset ~/ size.width)].idProducto;
            zonasss.zonaNombre =
                zona[(_scrollController.offset ~/ size.width)].zonaNombre;
            zonasss.zonaDescripcion =
                zona[(_scrollController.offset ~/ size.width)].zonaDescripcion;
            zonasss.zonaPedidoMinimo =
                zona[(_scrollController.offset ~/ size.width)].zonaPedidoMinimo;
            zonasss.zonaDescripcion =
                zona[(_scrollController.offset ~/ size.width)].zonaDescripcion;
            zonasss.zonaImagen =
                zona[(_scrollController.offset ~/ size.width)].zonaImagen;
                zonasss.route = argumento;
            Navigator.pushNamed(context, 'zoomDireccion', arguments: zonasss);
          },
          child: Text(
