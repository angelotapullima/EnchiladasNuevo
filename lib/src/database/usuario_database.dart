...
class UsuarioDatabase {
  ...
  insertarUsuariosDb(Userio user) async {
    final db = await dbprovider.database;

    final res = await db.rawInsert("INSERT or REPLACE INTO Usuario (c_u,idRel,nombre,email,foto,dni,id_direccion,id_zona,token,telefono,c_p) "
  ...
  Future<List<Userio>> obtenerUsUario() async {
    final db = await dbprovider.database;
    final res = await db.rawQuery("SELECT * FROM Usuario");

    List<Userio> list = res.isNotEmpty ? res.map((c) => Userio.fromJson(c)).toList() : [];
